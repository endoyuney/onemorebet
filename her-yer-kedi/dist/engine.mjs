...
export const TOYS={
 ...
 scratch:{name:'Tırmalama',icon:'🪵',cost:E.toys.scratch,skill:'scratch',hint:'İleri oyuncak · tahta + sörf'},
 box:{name:'Saklanma kutusu',icon:'📦',cost:E.toys.box,hint:'Kalıcı · ilk oyuncak ödülüne +1'},
...
export const SKILLS=[
 ...
 {id:'scratch',branch:1,name:'Patiden sörfe',desc:'1000 altınlık tahta: 25 tırmalama +1120; sörfte üç tur, tur başına +70.',cost:3500,req:['house','rod'],gate:s=>s.stats.feather>=5,need:'5 tüy yakala'},
 ...
 {id:'crowd',branch:2,name:'Yer açın!',desc:'Oyuncak sınırı oda başına 24 → 40. Toplam 8 kediye yer aç.',cost:4000,req:['room'],gate:s=>true,need:''}
];
...
export class Game{
 ...
 place(kind,x,y,room=this.s.active,free=false){const r=this.s.rooms[room],cost=this.cost(kind);if(!r||!this.available(kind)||r.toys.length>=this.capacity()||(!free&&this.s.gold<cost))return false;if(FIXTURES[kind]&&r.toys.filter(t=>t.kind===kind).length>=FIXTURES[kind].limit)return false;if(FIXTURES[kind]){const p=this.fixturePosition(kind,x,y,room);if(!p)return false;x=p.x;y=p.y;}if(!free)this.spend(cost,'toys');this.spawn(kind,x,y,room);return true;}
 ...
 target(cat){let best=null,score=Infinity;const localToys=this.s.rooms[cat.room].toys.filter(t=>!FIXTURES[t.kind]).length;
  for(let ri=0;ri<this.s.rooms.length;ri++)for(const t of this.s.rooms[ri].toys){
   ...
   if(FIXTURES[t.kind]&&(ri!==cat.room||this.s.time-cat.lastFixture[t.kind]<FIXTURES[t.kind].cooldown||(localToys>0&&this.s.time-cat.lastComfort<60)||(cat.plays<4&&localToys>0&&cat.lastFixture[t.kind]>-99)))continue;
   ...
   if(FIXTURES[t.kind])d-=cat.plays>=4?700:cat.lastFixture[t.kind]<=-99?240:50;
 ...
 for(let i=0;i<s.cats.length;i++)for(let j=i+1;j<s.cats.length;j++){const a=s.cats[i],b=s.cats[j];if(a.room!==b.room||['fixture','tunnel'].includes(a.mode)||['fixture','tunnel'].includes(b.mode))continue;const dx=a.x-b.x,dy=a.y-b.y,d=Math.hypot(dx,dy);if(d>0&&d<29){const f=(29-d)*dt*1.6;a.x+=dx/d*f;b.x-=dx/d*f;a.y+=dy/d*f;b.y-=dy/d*f;}}
 for(const c of s.cats){if(['fixture','tunnel'].includes(c.mode))continue;const p={x:c.x,y:c.y,z:0,vx:0,vy:0};for(const rect of FURNITURE)collideFurniture(p,rect,24);c.x=clamp(p.x,48,W-48);c.y=clamp(p.y,130,H-65);}
 }
 save(){return JSON.stringify(this.s);}
 static load(text){let s;try{s=JSON.parse(text);}catch{return null;}if(!s||![1,2,3,4].includes(s.version)||!Array.isArray(s.rooms)||s.rooms.length<1||s.rooms.length>2||!Array.isArray(s.cats)||s.cats.length<1||s.cats.length>8)return null;
 ...
 for(const r of s.rooms){if(!r||!Array.isArray(r.toys)||r.toys.length>40||!finite(r.bonusCarry,0,1)||!finite(r.autoTimer,0,E.autoSeconds))return null;for(const t of r.toys)if(!t||!validId(t.id)||!kinds.includes(t.kind)||!finite(t.x,0,W)||!finite(t.y,0,H)||!finite(t.vx,-1000,1000)||!finite(t.vy,-1000,1000)||!finite(t.z,0,1000)||!finite(t.vz,-1000,1000)||!finite(t.age,0,1e10)||!finite(t.count,0,100)||!finite(t.level,1,3)||!finite(t.phase,0,1e12)||!finite(t.value,1,15)||!finite(t.buildFrom,1,3)||!finite(t.buildProgress,0,1)||t.done)return null;}
 for(let i=0;i<s.cats.length;i++){const c=s.cats[i];if(!c||!validId(c.id)||c.index!==i||!s.rooms[c.room]||!finite(c.x,0,W)||!finite(c.y,0,H)||!finite(c.timer,-1e10,1e10)||!finite(c.plays,0,1e12)||!finite(c.lastNap,-1e10,1e10)||!finite(c.work,0,10)||!finite(c.tx,0,W)||!finite(c.ty,0,H)||!finite(c.phase,0,1e12)||![1,-1].includes(c.face)||!['idle','wander','notice','chase','play','rest','watch','fixture','tunnel'].includes(c.mode)||![null,0,1].includes(c.tunnelSide))return null;if(typeof c.lookAway!=='boolean'||!finite(c.lastComfort,-100,s.time)||!c.lastFixture||Object.keys(FIXTURES).some(k=>!finite(c.lastFixture[k],-100,s.time)))return null;if(c.mode==='fixture'){const f=c.fixture;if(!f||!FIXTURES[f.kind]||!finite(f.elapsed,0,fixtureDuration(f.kind))||!finite(f.fromX,0,W)||!finite(f.fromY,0,H)||!finite(f.exitX,0,W)||!finite(f.exitY,0,H)||!s.rooms[c.room].toys.some(t=>t.id===c.target&&t.kind===f.kind))return null;}if(c.mode==='tunnel'){const tr=c.transit;if(!tr||!finite(tr.elapsed,0,1.4)||tr.duration!==1.4||!finite(tr.fromX,0,W)||!finite(tr.toX,0,W)||!finite(tr.fromY,0,H)||!finite(tr.toY,0,H)||!s.rooms[c.room].toys.some(t=>t.id===c.target&&t.kind==='tunnel'))return null;}}
