...
export const TOYS={ball:{name:'Tenis topu',icon:'🎾',cost:1,hint:'Kısa sekme · +3 altın'},football:{name:'Futbol topu',icon:'⚽',cost:2,skill:'balls',hint:'Uzun yuvarlanma · +5'},basketball:{name:'Basketbol',icon:'🏀',cost:3,skill:'balls',hint:'Yüksek sekme · +7'},bed:{name:'Kedi yatağı',icon:'🛏️',cost:12,hint:'Kalıcı · uyku +2 · oda başına 2'},house:{name:'Kedi evi',icon:'🏠',cost:6,skill:'house',hint:'Devrilir, tünel olur'},rod:{name:'Olta',icon:'🎣',cost:7,skill:'rod',hint:'Kırılır, tüyü kaçar'},scratch:{name:'Tırmalama',icon:'🪵',cost:10,skill:'scratch',hint:'25 tırmalama → sörf'}};
...
export const SKILLS=[
 {id:'mice',branch:0,name:'İçinden fare çıktı!',desc:'Her top yakalanınca 2 fareye dönüşür. Fare başına +2 altın.',cost:12,req:[],gate:s=>s.stats.ball>=5,need:'5 top yakala'},
 {id:'surprise',branch:0,name:'Sürpriz paket',desc:'Toplar artık %50 iki fare, %25 frizbi, %25 üç böcek çıkarır.',cost:35,req:['mice'],gate:s=>s.stats.mouse>=6,need:'6 fare yakala'},
 {id:'rod',branch:0,name:'Tüyün peşinde',desc:'Oltayı aç. 6 hamlede kırılır (+10); kaçan tüyü yakala (+4).',cost:30,req:['mice'],gate:s=>true,need:''},
 {id:'balls',branch:0,name:'Topun karakteri',desc:'Futbol ve basketbol topları: farklı sekme, yuvarlanma ve ödüller.',cost:22,req:['mice'],gate:s=>true,need:''},
 {id:'house',branch:1,name:'Karton mimarisi',desc:'Kedi evini aç. Devrilince altın verir ve 10 geçişlik tünel olur.',cost:18,req:[],gate:s=>s.cats.length>=2,need:'2 kedi edin'},
 {id:'floor2',branch:1,name:'İkinci kat',desc:'Yeni evler 2 katlı. Devrilme ve tünel ödülleri iki katına çıkar.',cost:32,req:['house'],gate:s=>s.stats.house>=2,need:'2 ev devir'},
 {id:'floor3',branch:1,name:'Üçüncü kat',desc:'Yeni evler 3 katlı. Daha çok kedi etkileşebilir; üç kat ödül.',cost:65,req:['floor2'],gate:s=>s.stats.tunnel>=10,need:'10 tünel geçişi'},
 {id:'scratch',branch:1,name:'Patiden sörfe',desc:'Tahta 25 tırmalamada parçalanır (+14). Sörfte 3 tur daha: tur başına +3.',cost:45,req:['house'],gate:s=>true,need:''},
 {id:'room',branch:2,name:'Yan odanın kapısı',desc:'Bitişik oda ve bir yeni kedi. Toplar ve kediler açık kapıdan geçebilir.',cost:85,req:['house'],gate:s=>s.stats.tunnel>=10,need:'10 tünel geçişi'},
 {id:'auto',branch:2,name:'Top yağmuru',desc:'Her odada 7 saniyede bir top. Her top 1 altın; son 2 altın korunur.',cost:60,req:['mice'],gate:s=>s.cats.length>=3,need:'3 kedi edin'},
 {id:'crowd',branch:2,name:'Yer açın!',desc:'Oda başına oyuncak sınırı 24 → 40. Aynı anda daha büyük zincirler.',cost:70,req:['room'],gate:s=>true,need:''}
];
...
export class Game{
 ...
 addCat(room){const index=this.s.cats.length;if(index>=CATS.length)return false;const cat={id:this.s.nextId++,index,room,x:180+this.rand()*540,y:210+this.rand()*220,tx:450,ty:300,target:null,mode:'idle',timer:.5+this.rand(),work:0,face:1,phase:this.rand()*6,plays:0,lastNap:-20};this.s.cats.push(cat);this.emit({type:'cat',text:CATS[index][0]+' eve geldi!',room});return cat;}
 catCost(){return[0,10,24,44,68,96,135,180][this.s.cats.length]??Infinity;}
 adopt(){const cost=this.catCost();if(this.s.gold<cost||this.s.cats.length>=8)return false;this.s.gold-=cost;this.addCat(this.s.active);this.check();return true;}
 capacity(){return this.s.skills.crowd?40:24;}
 canBuy(id){const n=SKILLS.find(x=>x.id===id);return !!n&&!this.s.skills[id]&&n.req.every(k=>this.s.skills[k])&&n.gate(this.s)&&this.s.gold>=n.cost;}
 buySkill(id){if(!this.canBuy(id))return false;const n=SKILLS.find(x=>x.id===id);this.s.gold-=n.cost;this.s.skills[id]=true;if(id==='room'){this.s.rooms.push({name:'Oyun odası',toys:[],autoTimer:0});if(!this.addCat(1)){const c=this.s.cats.at(-1);c.room=1;c.target=null;c.mode='idle';c.timer=0;this.emit({type:'cat',text:CATS[c.index][0]+' yan odaya taşındı.',room:1});}}if(id==='auto')this.s.auto=true;this.emit({type:'skill',text:n.name+' açıldı!'});this.check();return true;}
 ...
 place(kind,x,y,room=this.s.active,free=false){const r=this.s.rooms[room],cost=TOYS[kind]?.cost;if(!r||!this.available(kind)||r.toys.length>=this.capacity()||(!free&&this.s.gold<cost))return false;if(kind==='bed'&&r.toys.filter(t=>t.kind==='bed').length>=2)return false;if(!free)this.s.gold-=cost;this.spawn(kind,x,y,room);return true;}
 ...
 starterBall(x=450,y=300){if(this.s.time<this.s.freeAt)return false;if(!this.place('ball',x,y,this.s.active,true))return false;this.s.freeAt=this.s.time+20;return true;}
 spawn(kind,x,y,room,extra={}){const r=this.s.rooms[room];if(r.toys.length>=this.capacity())return null;const angle=this.rand()*Math.PI*2,pos=safeFloor(x,y),level=kind==='house'?(this.s.skills.floor3?3:this.s.skills.floor2?2:1):1;
 const t={id:this.s.nextId++,kind,x:pos.x,y:pos.y,z:BALLS[kind]?22:0,vz:BALLS[kind]?.launch||0,vx:Math.cos(angle)*85,vy:Math.sin(angle)*85,age:0,count:0,level,done:false,phase:this.rand()*6,...extra};r.toys.push(t);return t;}
 reward(n,t,room,label){this.s.gold+=n;this.s.earned+=n;this.emit({type:'reward',amount:n,x:t.x,y:t.y,room,text:label});}
 complete(cat,t){const s=this.s,room=cat.room;if(t.done||!s.rooms[room].toys.includes(t)||(BALLS[t.kind]&&t.z>18))return false;
 ...
 if(BALLS[t.kind]){
  retire();s.stats.ball++;if(t.kind!=='ball')s.stats[t.kind]++;pop(BALLS[t.kind].reward,TOYS[t.kind].name+' yakalandı');
  if(s.skills.mice){let outcome=s.skills.surprise?this.rand():0;if(outcome<.5){transform('mouse',{vx:-110});transform('mouse',{vx:110});this.emit({type:'transform',text:'Top → iki fare!',room,x:t.x,y:t.y});}else if(outcome<.75){transform('frisbee');this.emit({type:'transform',text:'Top → frizbi!',room,x:t.x,y:t.y});}else{for(let i=0;i<3;i++)transform('bug');this.emit({type:'transform',text:'Top → üç zıpzıp!',room,x:t.x,y:t.y});}}
 }else if(['mouse','bug','feather'].includes(t.kind)){
  retire();s.stats[t.kind]++;pop(t.kind==='feather'?4:t.kind==='bug'?1:2,t.kind==='feather'?'Tüy yakalandı':t.kind==='mouse'?'Fare yakalandı':'Zıpzıp yakalandı');
 }else if(t.kind==='house'){
  t.count++;this.emit({type:'hit',room,x:t.x,y:t.y});if(t.count>=3+t.level){retire();s.stats.house++;pop(8*t.level,'Kedi evi devrildi');transform('tunnel',{level:t.level,vx:0,vy:0});this.emit({type:'transform',text:'Kedi evi → tünel!',room,x:t.x,y:t.y});}
 }else if(t.kind==='tunnel'){
  t.count++;s.stats.tunnel++;cat.x=clamp(t.x+(cat.face>0?72:-72),55,W-55);cat.y=t.y;this.emit({type:'pass',room,x:t.x,y:t.y,text:t.count+'/10 geçiş'});if(t.count>=10){retire();pop(6*t.level,'Tünel turu tamamlandı');}
 }else if(t.kind==='rod'){
  t.count++;if(t.count>=6){retire();s.stats.rod++;pop(10,'Olta kırıldı');transform('feather');this.emit({type:'transform',text:'Olta → kaçan tüy!',room,x:t.x,y:t.y});}
 }else if(t.kind==='scratch'){
  t.count+=cat.index===4?2:1;s.stats.scratch++;if(t.count>=25){retire();pop(14,'Tahta parçalandı');transform('surf');this.emit({type:'transform',text:'Tahta → sörf!',room,x:t.x,y:t.y});}
 }else if(t.kind==='surf'){
  t.count++;s.stats.surf++;pop(3,'Bir tur pati sörfü');cat.x=clamp(cat.x+cat.face*130,55,W-55);if(t.count>=3)retire();else{t.x=cat.x;t.y=cat.y;}
 }else if(t.kind==='frisbee'){
  t.count++;s.stats.frisbee++;pop(2,'Frizbi pası');if(t.count>=3)retire();else{t.x=clamp(t.x+cat.face*180,65,W-65);t.y=clamp(t.y+(this.rand()-.5)*150,125,H-75);t.age=0;}
 }
 ...
 step(dt){if(!Number.isFinite(dt)||dt<=0)return;dt=Math.min(dt,.1);const s=this.s;s.time+=dt;
 for(let ri=0;ri<s.rooms.length;ri++){const r=s.rooms[ri];if(s.skills.auto&&s.auto){r.autoTimer+=dt;if(r.autoTimer>=7){r.autoTimer=0;if(s.gold>=3&&r.toys.length<Math.min(12,this.capacity()))this.place('ball',100+this.rand()*700,210+this.rand()*290,ri);}}}
 const snapshot=s.rooms.flatMap((r,ri)=>r.toys.map(t=>({t,ri})));
 for(const {t,ri} of snapshot){t.age+=dt;t.phase+=dt*4;const held=s.cats.some(c=>c.room===ri&&c.target===t.id&&c.mode==='play');
 ...
 for(const c of s.cats){c.phase+=dt*9;const r=s.rooms[c.room];if(c.mode==='sleep'){c.timer-=dt;if(c.timer<=0){const bed=this.findToy(c.target);if(bed?.toy.kind==='bed'){this.reward(2,bed.toy,c.room,'Güzel bir pati uykusu');s.stats.naps++;}c.lastNap=s.time;c.plays=0;c.target=null;c.mode='rest';c.timer=.6;this.check();}continue;}if(c.mode==='rest'){c.timer-=dt;if(c.timer<=0)c.mode='idle';continue;}
  let found=this.findToy(c.target),t=found?.toy;
  if(!t){c.target=null;c.work=0;c.timer-=dt;if(c.timer<=0){t=this.target(c);if(t){c.target=t.id;c.mode='notice';c.timer=(c.index%3===2?.45:.16)+this.rand()*.2;}else{c.mode='wander';c.tx=80+this.rand()*740;c.ty=135+this.rand()*370;c.timer=2+this.rand()*3;}}}
 ...
 for(let i=0;i<s.cats.length;i++)for(let j=i+1;j<s.cats.length;j++){const a=s.cats[i],b=s.cats[j];if(a.room!==b.room||a.mode==='sleep'||b.mode==='sleep')continue;const dx=a.x-b.x,dy=a.y-b.y,d=Math.hypot(dx,dy);if(d>0&&d<29){const f=(29-d)*dt*1.6;a.x+=dx/d*f;b.x-=dx/d*f;a.y+=dy/d*f;b.y-=dy/d*f;}}
 for(const c of s.cats){if(c.mode==='sleep')continue;const p={x:c.x,y:c.y,z:0,vx:0,vy:0};for(const rect of FURNITURE)collideFurniture(p,rect,24);c.x=clamp(p.x,48,W-48);c.y=clamp(p.y,110,H-65);}
 }
 save(){return JSON.stringify(this.s);}
 static load(text){let s;try{s=JSON.parse(text);}catch{return null;}if(!s||![1,2].includes(s.version)||!Array.isArray(s.rooms)||s.rooms.length<1||s.rooms.length>2||!Array.isArray(s.cats)||s.cats.length<1||s.cats.length>8)return null;
 ...
 for(const r of s.rooms){if(!r||!Array.isArray(r.toys)||r.toys.length>40||!finite(r.autoTimer,0,8))return null;for(const t of r.toys)if(!t||!validId(t.id)||!kinds.includes(t.kind)||!finite(t.x,0,W)||!finite(t.y,0,H)||!finite(t.vx,-1000,1000)||!finite(t.vy,-1000,1000)||!finite(t.z,0,1000)||!finite(t.vz,-1000,1000)||!finite(t.age,0,1e10)||!finite(t.count,0,100)||!finite(t.level,1,3)||!finite(t.phase,0,1e12)||t.done)return null;}
 for(let i=0;i<s.cats.length;i++){const c=s.cats[i];if(!c||!validId(c.id)||c.index!==i||!s.rooms[c.room]||!finite(c.x,0,W)||!finite(c.y,0,H)||!finite(c.timer,-1e10,1e10)||!finite(c.plays,0,1e12)||!finite(c.lastNap,-1e10,1e10)||!finite(c.work,0,10)||!finite(c.tx,0,W)||!finite(c.ty,0,H)||!finite(c.phase,0,1e12)||![1,-1].includes(c.face)||!['idle','wander','notice','chase','play','rest','watch','sleep'].includes(c.mode))return null;}
